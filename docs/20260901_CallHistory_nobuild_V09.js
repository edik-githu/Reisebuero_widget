/*
 * Call-History Widget  |  Version V09 (no-build/TEST)  |  20260901  (Mehrtages-TEST: contactHistory statt agentHistory)
...
(function () {
  ...
  W.prototype._size= function(){ var v = parseInt(this.pageSize || this.getAttribute("page-size"),10); return isNaN(v)||v<=0 ? 500 : v; };
  ...
  W.prototype.load = function(){
    ...
    var daysBack = parseInt(this.getAttribute("days-back") || "30", 10) || 30;
    var to = Date.now(), from = to - daysBack * DAY_MS;
    // TEST: contactHistory liefert bis ~60 Tage (agentHistory war auf 24h gedeckelt).
    // Abfrage nur mit agentId (ohne ani) -> Ziel: eigene Mehrtages-Historie.
    var url = this._api() + "/v1/history/contactHistory?agentId=" + encodeURIComponent(agentId) +
      "&page=0&pageSize=" + this._size() + "&from=" + from + "&to=" + to;

    fetch(url, { credentials: "include", headers: { "Accept":"application/json", "X-Organization-Id":orgId, "Authorization":"Bearer "+token } })
  ...
  W.prototype._norm = function(r){
    ...
    var talk = r.talkDuration != null ? r.talkDuration : (r.talkTimeMs != null ? r.talkTimeMs : ((r.cetts && r.cstts) ? (Number(r.cetts) - Number(r.cstts)) : 0));
